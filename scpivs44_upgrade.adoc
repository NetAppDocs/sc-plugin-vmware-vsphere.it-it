...
---
= Aggiornamento da una versione precedente di SCV
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: L'aggiornamento a SCV 6.2 è supportato solo su VMware vCenter Server 7 update 1 e versioni successive, per VMware vCenter Server precedenti alla versione 7 update 1, è necessario continuare a utilizzare SCV 4.7. L'aggiornamento è dirompente sulle versioni non supportate di VMware vCenter Server.
...
CAUTION: Prima di eseguire l'upgrade, verifica che il tuo ambiente soddisfi i requisiti di sistema. Consulta link:scpivs44_deployment_planning_and_requirements.html["SnapCenter Plug-in per VMware vSphere pianificazione della distribuzione e requisiti"] per i dettagli.
...
| SCV 6,1 | Aggiorna a SCV 6.2 


| DISTRIBUTORE IDRAULICO 6,0 | Aggiorna a SCV 6.1 e SCV 6.2 
...
Per le informazioni più recenti sulle versioni supportate, consulta link:https://imt.netapp.com/imt/imt.jsp?components=180121;&solution=1517&isHWU&src=IMT["Tool di matrice di interoperabilità NetApp (IMT)"^].

.Fasi
. Preparati per l'aggiornamento disattivando il plug-in SnapCenter per VMware vSphere.
+
.. Accedi all'interfaccia utente di gestione SnapCenter Plug-in for VMware vSphere .  L'indirizzo IP viene visualizzato quando si distribuisce il SnapCenter Plug-in for VMware vSphere.
...
+
.. Accedi al link:https://mysupport.netapp.com/products/index.html["Sito di supporto NetApp"^].
...
.. Nell'interfaccia utente di vCenter, selezionare *vSphere Client (HTML)*.
