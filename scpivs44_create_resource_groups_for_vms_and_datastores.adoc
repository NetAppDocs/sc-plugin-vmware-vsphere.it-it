...
.Fasi
. Nel riquadro sinistro del Navigator del plug-in SCV, fare clic su *Resource Groups*, quindi su image:scpivs44_image6.png["Errore: Immagine grafica mancante"] *Crea* per avviare la procedura guidata.
...
+ ** per creare un gruppo di risorse per una macchina virtuale, fare clic su *Menu* > *host e cluster*, quindi fare clic con il pulsante destro del mouse su una macchina virtuale, selezionare NetApp SnapCenter e fare clic su image:scpivs44_image6.png["Errore: Immagine grafica mancante"] *Crea gruppo di risorse*. ** Per creare un gruppo di risorse per un datastore, fare clic su *Menu* > *host e cluster*, quindi fare clic con il pulsante destro del mouse su un datastore, quindi selezionare *NetApp SnapCenter* e fare clic su image:scpivs44_image6.png["Errore: Immagine grafica mancante"] *Crea gruppo di risorse*.
...
+
Il plug-in SnapCenter per VMware vSphere non esegue controlli di compatibilità per tutte le versioni di ONTAP, ma solo per ONTAP 8.2.1 e versioni precedenti. Pertanto, vedere sempre https://imt.netapp.com/matrix/imt.jsp?components=108380;&solution=1257&isHWU&src=IMT["Tool di matrice di interoperabilità NetApp (IMT)"^] Per informazioni aggiornate sul supporto SnapCenter.
