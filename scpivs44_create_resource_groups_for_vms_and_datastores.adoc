...
---
= Crea gruppi di risorse per VM e datastore in SCV
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Per i volumi FlexGroup negli ambienti MetroCluster, se si utilizza ONTAP 9.8 o 9.9, riavviare il servizio SnapCenter Plug-in per VMware vSphere e risincronizzare le relazioni SnapMirror dopo uno switchover o uno switchback prima di eseguire il backup dei gruppi di risorse. In ONTAP 9.8, i backup potrebbero bloccarsi dopo uno switchback; questo problema è stato risolto in ONTAP 9.9.
...
| Server vCenter | Seleziona un vCenter Server. 
