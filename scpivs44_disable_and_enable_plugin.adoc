...
---
= Disabilitare e abilitare il SnapCenter Plug-in per VMware vSphere
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Accedi all'interfaccia utente di gestione SnapCenter Plug-in for VMware vSphere utilizzando il formato `\https://<OVA-IP-address>:8080` .  Accedi con il nome utente e la password di amministratore impostati al momento della distribuzione e con il token MFA generato tramite la console di manutenzione.
+
