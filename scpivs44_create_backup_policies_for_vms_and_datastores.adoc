...
* SnapMirror Business Continuity (SM-BC) consente ai servizi di business di continuare a funzionare anche in caso di guasto completo del sito, supportando il failover delle applicazioni in modo trasparente utilizzando una copia secondaria.
+

NOTE: SM-BC è supportato solo per gli archivi dati VMFS.

+
Per proteggere un datastore VMFS in un'implementazione SM-BC, in qualità di amministratore SnapCenter devi:

+
** Configurare cluster e mediatore come descritto nel report tecnico: https://www.netapp.com/pdf.html?item=/media/21888-tr-4878.pdf["TR-4878: Business continuity SnapMirror"].
** Aggiungi il volume associato al datastore VMFS al gruppo di coerenza e crea una relazione di data Protection utilizzando la policy di protezione _AutomatedFailOver_ tra due sistemi storage ONTAP.
...
NOTE: Per proteggere il datastore VMFS nella distribuzione di SM-BC, è necessario completare i prerequisiti indicati nella sezione _prima di iniziare_ e abilitare *Update SnapMirror after backup*.
