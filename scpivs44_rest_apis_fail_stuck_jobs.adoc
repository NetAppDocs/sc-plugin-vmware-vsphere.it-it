...
---
= Contrassegna i processi bloccati come falliti utilizzando le API REST in SCV
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
| `/4.1/jobs/{id}/failJobs` | Quando si passano gli ID dei lavori bloccati in uno stato di esecuzione, `failJobs` contrassegna quei lavori come falliti.  Per identificare i lavori bloccati in esecuzione, utilizzare l'interfaccia utente del monitor dei lavori per visualizzare lo stato di ogni lavoro e l'ID del lavoro. 
|===